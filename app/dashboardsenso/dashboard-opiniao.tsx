...
export function DashboardOpiniao({ loggedUser }: DashboardOpiniaoProps) {
  ...
  const totalFiltros = useMemo(
    () => Object.entries(opiniaoFilters)
      .filter(([key, value]) => key !== "limit" && key !== "offset" && hasActiveFilterValue(value))
      .length,
    [opiniaoFilters],
  );

  const perguntaOptions = useMemo(
    () => opiniao.resumo?.resultado ?? [],
    [opiniao.resumo],
  );

  const opcaoOptions = useMemo(() => {
    const perguntaId = String(opiniaoFilters.perguntaId ?? "");
    if (!perguntaId) {
      return [];
    }

    const perguntaSelecionada = perguntaOptions.find((item) => item.id === perguntaId);
    return perguntaSelecionada?.opcoes ?? [];
  }, [opiniaoFilters.perguntaId, perguntaOptions]);
          ...
          <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-5">
            ...
            <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-5">
              ...
              <label className="block text-sm text-slate-300">
                ...
                <input
                  value={String(opiniaoFilters.bairro ?? "")}
                  onChange={(event) => patchFilters("opiniao", { bairro: event.target.value, offset: 0 })}
                  className="h-11 w-full rounded-xl border border-white/15 bg-slate-950/65 px-3 text-sm text-slate-100"
                  placeholder="Bairro"
                />
              </label>
              ...
              <label className="block text-sm text-slate-300">
                ...
                <input
                  value={String(opiniaoFilters.nome ?? "")}
                  onChange={(event) => patchFilters("opiniao", { nome: event.target.value, offset: 0 })}
                  className="h-11 w-full rounded-xl border border-white/15 bg-slate-950/65 px-3 text-sm text-slate-100"
              ...
              <label className="block text-sm text-slate-300">
                ...
                <input
                  type="date"
                  value={String(opiniaoFilters.respondidoDe ?? "")}
                  onChange={(event) => patchFilters("opiniao", { respondidoDe: event.target.value, offset: 0 })}
                  className="h-11 w-full rounded-xl border border-white/15 bg-slate-950/65 px-3 text-sm text-slate-100"
                />
              </label>
              ...
              <label className="block text-sm text-slate-300 xl:col-span-2">
                ...
                <select
                  value={String(opiniaoFilters.perguntaId ?? "")}
                  onChange={(event) =>
                    patchFilters("opiniao", {
                      perguntaId: event.target.value,
                      opcaoRespostaId: "",
                      offset: 0,
                    })
                  }
                  className="h-11 w-full rounded-xl border border-white/15 bg-slate-950/65 px-3 text-sm text-slate-100"
                  disabled={perguntaOptions.length === 0}
                >
                  <option value="">Todas</option>
                  {perguntaOptions.map((pergunta) => (
                    <option key={pergunta.id} value={pergunta.id}>{pergunta.texto}</option>
                  ))}
                </select>
              </label>

              <label className="block text-sm text-slate-300 xl:col-span-2">
                <span className="mb-1 block">Opcao de resposta</span>
                <select
                  value={String(opiniaoFilters.opcaoRespostaId ?? "")}
                  onChange={(event) => patchFilters("opiniao", { opcaoRespostaId: event.target.value, offset: 0 })}
                  className="h-11 w-full rounded-xl border border-white/15 bg-slate-950/65 px-3 text-sm text-slate-100"
                  disabled={opcaoOptions.length === 0}
                >
                  <option value="">Todas</option>
                  {opcaoOptions.map((opcao) => (
              ...
              <label className="block text-sm text-slate-300">
                ...
                <select
                  value={String(opiniaoFilters.limit ?? 20)}
                  onChange={(event) => patchFilters("opiniao", { limit: Number(event.target.value), offset: 0 })}
                  className="h-11 w-full rounded-xl border border-white/15 bg-slate-950/65 px-3 text-sm text-slate-100"
                >
              ...
              <label className="flex items-center gap-3 rounded-xl border border-white/10 bg-slate-950/45 px-3 py-3 text-sm text-slate-300 xl:col-span-2">
                <input
                  type="checkbox"
                  checked={Boolean(opiniaoFilters.apenasComCoordenada)}
                  onChange={(event) => patchFilters("opiniao", { apenasComCoordenada: event.target.checked, offset: 0 })}
            ...
            <div className="rounded-2xl border border-white/10 bg-white/5 p-5">
              <div className="flex flex-wrap items-center justify-between gap-2">
                ...
                <span className="text-xs text-slate-400">
                  {opiniao.participantes.totalFiltrado} encontrados | página {Math.floor(Number(opiniaoFilters.offset ?? 0) / Number(opiniaoFilters.limit ?? 20)) + 1}
                </span>
              </div>
                  ...
                  <tbody>
                    {opiniao.participantes.respostas.map((item) => (
                      <tr key={item.id} className="border-b border-white/5 align-top">
                        <td className="px-2 py-2 text-xs text-slate-400 whitespace-nowrap">{formatDate(item.respondidoEm)}</td>
                        <td className="px-2 py-2">{item.participante?.nome || "-"}</td>
                        <td className="px-2 py-2 text-xs">{item.participante?.contatoOpcional || "-"}</td>
                        <td className="px-2 py-2 text-xs">{[item.bairro, item.cidade, item.estado].filter(Boolean).join(" - ") || "-"}</td>
                        <td className="px-2 py-2 text-xs">{item.entrevistador?.nome || item.entrevistador?.email || "-"}</td>
                        <td className="px-2 py-2">
                          {item.respostas && item.respostas.length > 0 ? (
                            <ul className="space-y-1">
                              {item.respostas.map((r, idx) => (
                                <li key={r.id ?? idx} className="text-xs text-slate-300">
                                  <span className="text-slate-400">{r.pergunta?.texto ?? "—"}:</span>{" "}
                                  <span className="font-medium text-cyan-200">{r.opcaoResposta?.texto ?? "—"}</span>
                                </li>
                              ))}
                            </ul>
                          ) : (
              ...
              <div className="mt-4 flex items-center gap-3">
                <Button
                  type="button"
                  disabled={Number(opiniaoFilters.offset ?? 0) === 0}
                  onClick={() =>
                    patchFilters("opiniao", {
                      offset: Math.max(0, Number(opiniaoFilters.offset ?? 0) - Number(opiniaoFilters.limit ?? 20)),
                    })
                  }
                  className="border border-white/20 bg-white/5 text-slate-200 disabled:opacity-40"
                >
                  Anterior
                </Button>
                <Button
                  type="button"
                  disabled={!opiniao.participantes.paginacao?.temMais}
                  onClick={() =>
                    patchFilters("opiniao", {
                      offset: Number(opiniaoFilters.offset ?? 0) + Number(opiniaoFilters.limit ?? 20),
                    })
                  }
                  className="border border-white/20 bg-white/5 text-slate-200 disabled:opacity-40"
                >
