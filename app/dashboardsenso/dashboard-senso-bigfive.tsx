...
export function DashboardSensoBigFive({ loggedUser }: DashboardSensoBigFiveProps) {
  ...
  const totalFiltrosSenso = useMemo(
    () => Object.keys(sensoFilters).filter((key) => sensoFilters[key] != null && String(sensoFilters[key]).trim() !== "").length,
    [sensoFilters],
  );

  const totalFiltrosBigFive = useMemo(
    () => Object.keys(bigFiveFilters).filter((key) => bigFiveFilters[key] != null && String(bigFiveFilters[key]).trim() !== "").length,
    [bigFiveFilters],
  );
