...
import { obterResumoSensoDashboardFromExternalApi } from "@/service/dashboard.service";
...
export async function GET(request: Request, context: { params: Promise<{ questionarioId: string }> }) {
  ...
  try {
    const result = await obterResumoSensoDashboardFromExternalApi(token, questionarioId, mapSearchParams(request));
    return NextResponse.json(result, { status: 200 });
  } catch (error) {
    return dashboardApiErrorResponse(error, "Falha ao carregar resumo do dashboard de senso.");
  }
}
